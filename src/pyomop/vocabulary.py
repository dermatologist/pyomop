...
class CdmVocabulary(object):
    ...
    def create_vocab(self, folder):
        df = pd.read_csv(folder + '/DRUG_STRENGTH.csv', error_bad_lines=False, warn_bad_lines=True)
        df.to_sql('drug_strength', con=self._engine, index = False, if_exists = 'replace')
        df = pd.read_csv(folder + '/CONCEPT.csv', error_bad_lines=False, warn_bad_lines=True)
        df.to_sql('concept', con=self._engine, index = False, if_exists = 'replace')
        df = pd.read_csv(folder + '/CONCEPT_RELATIONSHIP.csv', error_bad_lines=False, warn_bad_lines=True)
        df.to_sql('concept_relationship', con=self._engine, index = False, if_exists = 'replace')
        df = pd.read_csv(folder + '/CONCEPT_ANCESTOR.csv', error_bad_lines=False, warn_bad_lines=True)
        df.to_sql('concept_ancester', con=self._engine, index = False, if_exists = 'replace')
        df = pd.read_csv(folder + '/CONCEPT_SYNONYM.csv', error_bad_lines=False, warn_bad_lines=True)
        df.to_sql('concept_synonym', con=self._engine, index = False, if_exists = 'replace')
        df = pd.read_csv(folder + '/VOCABULARY.csv', error_bad_lines=False, warn_bad_lines=True)
        df.to_sql('vocabulary', con=self._engine, index = False, if_exists = 'replace')
        df = pd.read_csv(folder + '/RELATIONSHIP.csv', error_bad_lines=False, warn_bad_lines=True)
        df.to_sql('relationship', con=self._engine, index = False, if_exists = 'replace')
        df = pd.read_csv(folder + '/CONCEPT_CLASS.csv', error_bad_lines=False, warn_bad_lines=True)
        df.to_sql('concept_class', con=self._engine, index = False, if_exists = 'replace')
        df = pd.read_csv(folder + '/DOMAIN.csv', error_bad_lines=False, warn_bad_lines=True)
